...
class Search:
    def get_weibo(self,keyword):
        weibo_list = []
        weibo_list.extend(self.get_user(keyword))
        #i = 2
        #while i<=self.pagecount and i<=2:
    ...
    def get_user(self,keyword,page = 1):
        ...
        for li in lis:
            if li.text.strip() == '下一页' or li.text.strip() == '...':
                ...
                if self.pagecount < int(li.text.strip()):
                    self.pagecount = int(li.text.strip())
        return weibo_list_temp
        #if page == 1:
        #    pagecount = 1
        #    div_pagecount = soup.find('div',attrs={'class': "search_page clearfix"})
        #    lis = div_pagecount.findAll('li')
